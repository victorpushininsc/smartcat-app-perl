...
is($config->username, '__token_id__', "'username' attribute set as 'token_id' value");
is($config->password, '__token__', "'password' attribute set as 'token' value");
...
ok($config->username eq '__token_id__changed__', "'username' attribute saved properly");
ok($config->password eq '__token__changed__', "'password' attribute saved properly");
ok($config->log eq '__log__changed__', "'log' attribute saved properly");

move($test_config_path.".bak", $test_config_path);

$test_config_path = catfile(dirname(abs_path(__FILE__)), 'data', 'test_validate_log.config');
like(
    exception {
